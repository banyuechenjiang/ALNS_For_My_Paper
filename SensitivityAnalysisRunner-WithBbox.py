# SensitivityAnalysisRunner-WithBbox.py-v2 (修正配置路径)
"""
参数敏感性分析运行器 - 用于对比区域分割 (Bounding Box) 效果。
- 基于 SensitivityAnalysisRunner.py-v5-v2 (修正配置路径)
- 专门配置为测试 'buffer' 参数 (0: 关闭区域分割, 1: 开启)。
- 输出结果到 'sensitivity_analysis_results_bbox' 目录。
"""
import subprocess
import json
import itertools
...
# --- 配置 (修改为只测试 buffer 参数, 修正 base_config_path) ---
CONFIG_SENSITIVITY: Dict[str, Any] = {
    "parameter_ranges": {
        "buffer": [0, 1] #  测试 buffer 参数 (0: 关闭, 1: 开启)
    },
    "base_config_path": "base_experiment_config_with_bbox.json", # !!! 修正: 使用 with_bbox 版本 !!!
    "experiment_runner_script": "ExperimentRunner.py",
    "results_base_dir": "sensitivity_analysis_results_bbox", # 修改结果目录
    "scenario_to_run": 2,
    "num_instances_per_setting": 10, # 增加每个设置的实例数
    "run_alns_only": True,
...
# --- 辅助函数 (run_command, setup_analysis_directories, load_base_config, generate_temp_config_path - 不变，与 v5 相同) ---
...
def run_sensitivity_analysis(config: Dict[str, Any]):
    """执行完整的参数敏感性分析流程。"""
    print("====== 开始参数敏感性分析 (Buffer Parameter) ======")
    ...
    # 2. 加载基础配置
    base_config = load_base_config(config['base_config_path']) # !!! 确保加载 with_bbox 版本 !!!
    ...
    for param_name, param_values in parameters_to_test.items():
        ...
        for param_value in param_values:
            ...
            current_config[param_name] = param_value # 设置 buffer 参数
            ...
            # 控制 ER 内部 ALNS 的行为 (保持禁用历史记录和绘图以加速)
...
if __name__ == "__main__":
    ...
    if not FilePath(required_script).is_file():
         print(f"错误: 依赖的脚本 '{required_script}' 不存在。请确保它是 ExperimentRunner.py-v25 或更高版本。")
         sys.exit(1)
